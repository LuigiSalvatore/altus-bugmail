...
function getBranches(repo) {
  ...
  try {
    return JSON.parse(localStorage.getItem(k)) || [DEF_BRANCH];
  } catch (e) {
    return [DEF_BRANCH];
  }
}
...
function getRepos() {
  try {
    return JSON.parse(localStorage.getItem(LS_RE)) || [DEF_REPO];
  } catch (e) {
    return [DEF_REPO];
  }
}
...
function getDefBranch(repo) {
  return localStorage.getItem(
    `bz_def_branch_${(repo || cs.repo).replace(/[^a-zA-Z0-9_]/g, '_')}`
  ) || DEF_BRANCH;
}
...
function getFileHist() {
  try {
    return JSON.parse(localStorage.getItem(LS_FH)) || [];
  } catch (e) {
    return [];
  }
}
...
function getBugRefs() {
  ...
  if (!bug) {
    return cs.bugName || '{PLACEHOLDER}';
  }

  return `BUG-${bug.id} - ${bug.summary || ''}`.trim();
}

function getSingleChangedFile() {
  const files = cs.files.filter(f => f.file?.trim());

  if (files.length !== 1) {
    return null;
  }

  return files[0].file.trim();
}
  ...
  const rowsHTML = cs.files.map((f, i) => `
    <div class="file-row">
      <input
        type="text"
        class="form-input"
        list="file-history-datalist"
        value="${esc(f.file)}"
        placeholder="path/to/file.c"
        data-file-idx="${i}"
        data-file-prop="file">
      ...
      <button
        class="btn btn-ghost btn-icon btn-sm file-rm"
        data-remove-row="${i}"
        title="Remove"
        ${cs.files.length === 1 ? 'disabled' : ''}>
        ✕
      </button>
    </div>
  `).join('');
  ...
  return `
    <div class="cg2">
      <div class="form-group">
        ...
        <input
          type="text"
          class="form-input"
          value="${esc(cs.bugName)}"
          placeholder="Bug XXXXX - Summary"
          data-cs-field="bugName">
      </div>
      ...
      <div class="form-group">
        ...
        <input
          type="text"
          class="form-input"
          value="${esc(cs.bugLink)}"
          placeholder="https://…"
          data-cs-field="bugLink">
      </div>
    </div>
    ...
    <div class="form-group">
      <div class="fgh">
        ...
        <button class="btn btn-outline btn-sm" data-action="add-file-row">
          + Add Row
        </button>
      </div>

      <div class="file-rows-head">
        <span>File path</span>
        <span>Description of change</span>
        <span></span>
      </div>

      <div id="file-rows-${formId}">
        ${rowsHTML}
      </div>
    </div>
    ...
    <div class="cg2">
      <div class="form-group">
        <div class="fgh">
          ...
          <button class="btn btn-ghost btn-sm" data-manage-type="repo">
            ✎ Manage
          </button>
        </div>

        <select class="form-input" data-cs-select="repo">
          ${rOpts}
        </select>
      </div>
      ...
      <div class="form-group">
        <div class="fgh">
          ...
          <button class="btn btn-ghost btn-sm" data-manage-type="branch">
            ✎ Manage
          </button>
        </div>

        <select class="form-input" data-cs-select="branch">
          ${bOpts}
        </select>
      </div>
    </div>
    ...
    <div class="form-group">
      <div class="fgh" style="flex-wrap:wrap;gap:8px;">
        ...
        <div style="display:flex;gap:6px;">
          <button class="btn btn-outline btn-sm" data-action="copy-prompt">
            📋 Copy Prompt
          </button>

          <button class="btn btn-primary btn-sm" data-action="copy-and-gpt">
            🤖 Copy &amp; Go to GPT
          </button>
        </div>
      </div>
...
function _wireFormEvents() {
  ...
  document.querySelectorAll('[data-file-idx]').forEach(el => {
    el.addEventListener('input', () => {
      ...
      if (prop === 'file' && el.value.trim()) {
        addToFileHist(el.value.trim());
      }
  ...
  document.querySelectorAll('[data-manage-type]').forEach(btn => {
    btn.addEventListener('click', () => {
      openManageDrop(btn.dataset.manageType);
    });
  });
...
function updatePromptOutputs() {
  ...
  document.querySelectorAll('.commit-out').forEach(el => {
    el.textContent = p;
  });
}
...
function buildPrompt() {
  ...
  const changesStr = files.length
    ? files.map(f => `- [${f.file}] ${f.desc}`).join('\n')
    : '{PLACEHOLDER}';

  const singleFile = getSingleChangedFile();
...
<type>: <description>${singleFile ? ' (<file>)' : ''}
...
- if ONLY ONE file changed, append the filename/path to the FIRST LINE
- if multiple files changed, DO NOT mention files in the first line
- body: explain WHAT, WHY, HOW
- do NOT include a separate "Files:" section
- always include Refs using EXACTLY this format:
  Refs: ${getBugRefs()}
...
export function autoFillBug() {
  ...
  if (!bug) {
    toast('No current bug loaded', 'error');
    return;
  }
  ...
  cs.bugDesc = comments.length
    ? comments.map(c =>
      `--- ${c.author || '?'} (${(c.creation_time || '').slice(0, 10)}) ---\n${(c.text || '').trim()}`
    ).join('\n\n')
    : (bug.description || '');
...
function openManageDrop(type) {
  ...
  setTimeout(() => {
    document.getElementById('manage-dropdown-input').focus();
  }, 60);
}
...
function renderManageList() {
  const items = _mType === 'branch'
    ? getBranches(cs.repo)
    : getRepos();

  const def = _mType === 'branch'
    ? getDefBranch(cs.repo)
    : getDefRepo();
  ...
  document.querySelectorAll('[data-set-def]').forEach(btn => {
    btn.addEventListener('click', () => {
      _setManageDef(btn.dataset.setDef);
    });
  });

  document.querySelectorAll('[data-remove-manage]').forEach(btn => {
    btn.addEventListener('click', () => {
      _removeManageItem(parseInt(btn.dataset.removeManage));
    });
  });
}
...
export function initCommitSystem() {
  ...
  if (!cs.branch) {
    cs.branch = getDefBranch(cs.repo);
  }

  renderCommitForms();

  // Manage dropdown modal events
  document.getElementById('manage-dropdown-input')
    .addEventListener('keydown', e => {
      if (e.key === 'Enter') {
        _addManageDropdownItem();
      }
    });

  document.getElementById('manage-dropdown-overlay')
  ...
  // Done button
  document.getElementById('btn-manage-done')
    .addEventListener('click', closeManageDropdown);

  // Add button
  document.getElementById('btn-manage-add')
    .addEventListener('click', _addManageDropdownItem);

  // Fill from current bug
  document.getElementById('btn-fill-from-bug')
    .addEventListener('click', autoFillBug);
}